...
description: >
  ...
  when Codex needs to fix build errors, fix failing tests, perform multi-step
...
3. **Fail-safe guards are non-negotiable** — Loops exit on: max iterations reached, no progress detected, critical error encountered, more errors introduced than fixed, or user interruption. These guards exist to prevent the most common failure mode: Codex stubbornly retrying the same broken approach 20 times.
...
*Codex tries 47 iterations, burns through context window,
