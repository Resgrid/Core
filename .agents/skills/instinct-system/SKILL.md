...
description: >
  ...
  in .Codex/instincts.md. Load this skill when you notice a recurring pattern,
...
Store instincts in `.Codex/instincts.md` with categories and structured metadata:
...
5. Update .Codex/instincts.md with new count and confidence
...
2. If user approves:
   ...
   - Remove from .Codex/instincts.md
...
Export instincts at 0.7+ to `.Codex/instincts-export.md`. On import, apply 0.2 confidence decay (0.9 becomes 0.7, 0.7 becomes 0.5). Never import above 0.7 — every project must confirm locally. Mark imported instincts with `source: "imported from [project]"`.
...
```
SESSION START:
1. Read .Codex/instincts.md (if it exists)
...
.Codex/instincts.md has 40 instincts, 35 of them at confidence 0.3
...
.Codex/instincts.md grows to 200 lines, full of contradictions
