...
description: >
  ...
  Codex's mistake rate drops over time by compounding corrections into permanent
  knowledge. Load this skill when a user corrects Codex's output, mentions
...
5. **Review memory at session start** — The first thing Codex should do in a new session is check `MEMORY.md` for project-specific rules. Knowledge captured but never reviewed is wasted effort.

## Patterns

### Correction Detection & Capture Flow

When a user corrects Codex's output, follow this exact sequence:
...
```
// BAD — user corrects, Codex fixes but doesn't capture
User: "No, we use HybridCache here, not IMemoryCache"
Codex: "Fixed. Here's the updated code with HybridCache..."
...
Codex doesn't read it because it's too long to be useful

// GOOD — periodic audit keeps memory lean and accurate
MEMORY.md stays under 80 rules, well-categorized, no duplicates,
Codex reads it at session start and applies rules proactively
```
...
| Scenario | Action |
|----------|--------|
| User explicitly corrects Codex's code | Capture generalized rule in MEMORY.md |
