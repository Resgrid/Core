...
description: >
  ...
  "10x", "setup Codex", or "speed up development".
...
5. **Compound your knowledge** — Every correction becomes a rule in `MEMORY.md` (see `self-correction-loop` skill). Every PR review adds a learning. Over time, Codex's mistake rate drops because your project's knowledge base grows.
...
Boris's #1 tip: "Give Codex a way to verify its work." The short version: always tell Codex to run `dotnet build`, `dotnet test`, `get_diagnostics`, and `dotnet format --verify-no-changes` before declaring done. The verification-loop skill has the complete pipeline with short-circuit rules and report templates.
