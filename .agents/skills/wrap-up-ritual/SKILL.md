...
description: >
  ...
  and learnings before a session ends. Writes a handoff note to .Codex/handoff.md
...
4. **Consistent location, always overwritten** — The handoff file lives at `.Codex/handoff.md`. Each session overwrites the previous one — there's only ever one active handoff. Old handoffs are not valuable; current state is.
...
"Let me write a handoff note to .Codex/handoff.md so the next session
...
Codex: "You're welcome! Happy to help."
*Next session: "What were we working on? Let me read through the git log..."*

// GOOD — session ends with a handoff
User: "Thanks, that's all!"
Codex: "Before we wrap up, let me write a handoff note to .Codex/handoff.md
...
| Scenario | Action |
|----------|--------|
| User says "wrap up" / "that's all" / "done" | Write handoff to `.Codex/handoff.md` |
