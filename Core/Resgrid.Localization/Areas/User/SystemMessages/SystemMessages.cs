...
namespace Resgrid.Localization.Areas.User.SystemMessages
{
	/// <summary>Marker type used by ASP.NET Core localization for system message resources.</summary>
	public class SystemMessages
	{
	}

	/// <summary>
	/// Culture-explicit access to the account-level messages Resgrid sends to a single person —
	/// verification codes, data export notices, generic notification and calendar mail. Each is
	/// composed on a background or request thread for a recipient whose language is their own, so
	/// lookups take the recipient's culture rather than reading the ambient thread culture.
	/// A missing culture for a key falls back to English; a missing key surfaces the key name.
	/// </summary>
	public static class SystemMessagesResources
	{
		private static readonly ResourceManager ResourceManager = new ResourceManager(
			typeof(SystemMessages).FullName!, typeof(SystemMessages).Assembly);
