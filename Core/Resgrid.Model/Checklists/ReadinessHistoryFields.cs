...
namespace Resgrid.Model.Checklists
{
	public static class ReadinessHistoryFields
	{
		...
		public static readonly int[] AuditTypes = Enum.GetValues<AuditLogTypes>().Where(x => (x.ToString().StartsWith("Checklist", StringComparison.Ordinal) || x.ToString().StartsWith("WorkOrder", StringComparison.Ordinal) || x.ToString().StartsWith("Inventory", StringComparison.Ordinal))).Select(x => (int)x).ToArray();
