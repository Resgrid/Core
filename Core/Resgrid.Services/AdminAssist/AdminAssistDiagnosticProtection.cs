...
{
	public sealed class AdminAssistDiagnosticProtection(IEncryptionService encryption, IProtectedWriteService write,
		IProtectedReadService read, IDepartmentDataProtectionService protection, IProtectedGrantContext grant) : IAdminAssistDiagnosticProtection
	{
		private static readonly IReadOnlyDictionary<string, (Func<AdminAssistDiagnosticRun, string>, Action<AdminAssistDiagnosticRun, string>)> Fields =
			new Dictionary<string, (Func<AdminAssistDiagnosticRun, string>, Action<AdminAssistDiagnosticRun, string>)> { ["adminassistdiagnosticruns.content"] = (r => r.Content, (r, value) => r.Content = value) };
		private static string Binding(AdminAssistDiagnosticRun row) => "AdminAssistDiagnostic:" + row.UserId + ":" + row.Id;
		...
		public async Task ProtectAsync(AdminAssistActor actor, AdminAssistDiagnosticRun row, DiagnosticRequest request, CancellationToken ct)
		{
			...
			var result = await write.PrepareRecordsEntityWriteAsync(actor.DepartmentId, row, null, row.Id, Fields,
				() => { row.IsProtected = true; row.ProtectedCatalogVersion = 32; }, grant.GrantToken, actor.UserId, false, ct);
		...
		public async Task<DiagnosticRequest> ReadAsync(AdminAssistActor actor, AdminAssistDiagnosticRun row, CancellationToken ct)
		{
			if (row.DepartmentId != actor.DepartmentId || row.UserId != actor.UserId) throw new UnauthorizedAccessException();
			await RequireAsync(actor, ct);
			...
			try { return JsonSerializer.Deserialize<DiagnosticRequest>(encryption.DecryptForDepartment(row.Content, actor.DepartmentId, Binding(row))) ?? throw new UnauthorizedAccessException(); }
