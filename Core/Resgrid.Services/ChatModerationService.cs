...
namespace Resgrid.Services
{
	...
	public class ChatModerationService : IChatModerationService
	{
		...
		{
			var flags = await _chatMessageFlagRepository.GetByStatusAsync(departmentId, (int)status, Math.Max(page, 0), pageSize <= 0 ? 25 : Math.Min(pageSize, 100));
			return flags?.ToList() ?? new List<ChatMessageFlag>();
		}

		public async Task<ChatMessageFlag> ResolveFlagAsync(string chatMessageFlagId, int departmentId, string byUserId, ChatFlagStatus resolution, string resolutionNote, CancellationToken cancellationToken = default(CancellationToken))
		{
			...
			await RecordActionAsync(flag.DepartmentId, flag.ChatChannelId, flag.ChatMessageId, null, null,
				...
				AuditLogTypes.ChatFlagResolved, cancellationToken);

			return saved;
		}

		public async Task<bool> ModeratorDeleteMessageAsync(string chatMessageId, string byUserId, string reason, CancellationToken cancellationToken = default(CancellationToken))
		{
			var message = await _chatMessageRepository.GetByIdAsync(chatMessageId);
			if (message == null)
				return false;
			...
			await RecordActionAsync(message.DepartmentId, message.ChatChannelId, chatMessageId, message.SenderUserId, message.SenderUnitId,
				ChatModerationActionType.DeleteMessage, byUserId, reason, null, AuditLogTypes.ChatMessageDeletedByModerator, cancellationToken);

			return true;
		}

		public async Task<bool> SetUserMutedAsync(string chatChannelId, string targetUserId, DateTime? mutedUntil, string byUserId, string reason, CancellationToken cancellationToken = default(CancellationToken))
		{
			...
			await RecordActionAsync(channel.DepartmentId, chatChannelId, null, targetUserId, null,
				...
				muted ? AuditLogTypes.ChatUserMuted : AuditLogTypes.ChatUserUnmuted, cancellationToken);

			return true;
		}

		public async Task<bool> SetUserBannedAsync(string chatChannelId, string targetUserId, bool banned, string byUserId, string reason, CancellationToken cancellationToken = default(CancellationToken))
		{
			var channel = await _chatChannelRepository.GetByIdAsync(chatChannelId);
			if (channel == null)
				return false;
			...
			member.BannedOn = banned ? DateTime.UtcNow : (DateTime?)null;
			member.BannedByUserId = banned ? byUserId : null;
			member.ModifiedOn = DateTime.UtcNow;
			await _chatChannelMemberRepository.UpdateAsync(member, cancellationToken);
			...
			await RecordActionAsync(channel.DepartmentId, chatChannelId, null, targetUserId, null,
				...
				banned ? AuditLogTypes.ChatUserBanned : AuditLogTypes.ChatUserUnbanned, cancellationToken);

			return true;
		}

		public async Task<bool> SetChannelLockedAsync(string chatChannelId, bool locked, string byUserId, string reason, CancellationToken cancellationToken = default(CancellationToken))
		{
			var channel = await _chatChannelRepository.GetByIdAsync(chatChannelId);
			if (channel == null)
				return false;
			...
			channel.LockedByUserId = locked ? byUserId : null;
			channel.LockedOn = locked ? DateTime.UtcNow : (DateTime?)null;
			...
			await RecordActionAsync(channel.DepartmentId, chatChannelId, null, null, null,
				...
				locked ? AuditLogTypes.ChatChannelLocked : AuditLogTypes.ChatChannelUnlocked, cancellationToken);
		...
		{
			var actions = await _chatModerationActionRepository.GetByDepartmentAsync(departmentId, chatChannelId, Math.Max(page, 0), pageSize <= 0 ? 25 : Math.Min(pageSize, 100));
			return actions?.ToList() ?? new List<ChatModerationAction>();
		}

		public async Task<ChatExport> RequestExportAsync(int departmentId, string byUserId, string chatChannelId, DateTime? startDate, DateTime? endDate, ChatExportFormat format, CancellationToken cancellationToken = default(CancellationToken))
		{
			...
			await RecordActionAsync(departmentId, chatChannelId, null, null, null,
				...
				AuditLogTypes.ChatExportRequested, cancellationToken);
		...
		public async Task<ChatExport> GetExportForDownloadAsync(string chatExportId, int departmentId, string byUserId, CancellationToken cancellationToken = default(CancellationToken))
		{
			...
			await RecordActionAsync(departmentId, export.ChatChannelId, null, null, null,
				...
				AuditLogTypes.ChatExportDownloaded, cancellationToken);
			...
			ChatModerationActionType actionType, string byUserId, string reason, string detailsJson, AuditLogTypes auditLogType, CancellationToken cancellationToken)
		{
			...
			await _auditService.SaveAuditLogAsync(new AuditLog
			{
				...
				Data = JsonConvert.SerializeObject(new { chatChannelId, chatMessageId, targetUserId, targetUnitId, reason, detailsJson }),
