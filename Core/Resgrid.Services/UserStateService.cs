...
namespace Resgrid.Services
{
	public class UserStateService : IUserStateService
	{
		...
		public async Task<UserState> CreateUserState(string userId, int departmentId, int userStateType, CancellationToken cancellationToken = default(CancellationToken))
		{
			...
			// row key is the identity pk, then re-persists the enveloped row. A state note is often
			// why someone is unavailable, which is exactly the kind of free text the plan protects.
			...
			if (protectedWrite.Changed)
				saved = await _userStateRepository.SaveOrUpdateAsync(saved, cancellationToken);
		...
		public async Task<UserState> CreateUserState(string userId, int departmentId, int userStateType, string note, CancellationToken cancellationToken = default(CancellationToken), bool autoGenerated = false)
		{
			var us = new UserState();
			us.UserId = userId;
			us.DepartmentId = departmentId;
			...
			var previousStaffing = await _userStateRepository.GetPreviousUserStateByUserIdAsync(userId, saved.UserStateId);
			_eventAggregator.SendMessage<UserStaffingEvent>(new UserStaffingEvent() { DepartmentId = departmentId, Staffing = saved, PreviousStaffing = previousStaffing, AutoGenerated = autoGenerated });
			InvalidateLatestStatesForDepartmentCache(departmentId);
