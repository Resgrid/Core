...
namespace Resgrid.Services
{
    public sealed partial class WorkOrdersService
    {
        ...
        public async Task<WorkOrderBulkResult> PreviewBulkAsync(ChecklistActor actor, WorkOrderBulkInput input)
        {
            ...
            var result = new WorkOrderBulkResult(); var seen = new HashSet<string>(); var captured = new Dictionary<string, WorkOrder>();
            foreach (var row in input.Rows)
            {
                ...
                try
                {
                    ...
                    if (row.Import != null)
                    {
                        if (row.WorkOrderId != null || row.Assignment != null || row.Import.Content?.Approval != null) throw new WorkOrderException(400, "BulkInputInvalid");
                        ...
                        row.Import.Content.Currency = await DepartmentCurrencyAsync(actor.DepartmentId);
                        if (existing != null)
                        {
                            captured[existing.Id] = await ReadOrderAsync(actor, existing.Id); await RevealAsync(actor, existing);
                            row.Import.Content.Currency = Decode<StoredContent>(existing.Content).Fields.Currency;
                    ...
                    {
                        if (row.WorkOrderId == null || row.Assignment == null || !seen.Add(row.WorkOrderId)) throw new WorkOrderException(400, "BulkInputInvalid");
                        var order = await ReadOrderAsync(actor, row.WorkOrderId); captured[order.Id] = order;
        ...
        public async Task<WorkOrderBulkResult> ApplyBulkAsync(ChecklistActor actor, WorkOrderBulkInput input)
        {
            ...
            foreach (var item in result.Rows.Where(r => r.ErrorCode == null && !r.Applied))
            {
                ...
                try
                {
                    if (row.Import != null) item.WorkOrderId = (await CreateAsync(actor, row.Import, row.Import.Content.Currency)).Order.Id;
                    else await TransactionAsync(actor, async events =>
                    {
                        var receipt = (await _maintenance.QueryMaintenanceAsync<WorkOrderOperationReceipt>(actor.DepartmentId, "RequestId", BulkRowId(input, row))).SingleOrDefault();
                        if (receipt != null) { await ReadOrderAsync(actor, row.WorkOrderId); await ValidateBulkReceiptAsync(actor, input, row, receipt); return true; }
                        await AssignWithinAsync(actor, row.WorkOrderId, row.Assignment, events);
