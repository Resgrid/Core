...
namespace Resgrid.Providers.GeoLocationProvider
{
	public class GeoLocationProvider : IGeoLocationProvider
	{
		...
		public string GetAproxAddressFromLatLong(double lat, double lon)
		{
			return GetAddressFromLatLong(lat, lon);
		}
		...
		public RouteInformation GetRoute(double startLat, double startLon, double endLat, double endLon)
		{
			...
			Func<RouteInformation> getRoute = delegate ()
			{
			...
			return _cacheProvider.Retrieve<RouteInformation>(string.Format(RouteCacheKey, string.Format("{0}{1}", startLat, startLon).GetHashCode(), string.Format("{0}{1}", endLat, endLon).GetHashCode()), getRoute, CacheLength);
		}

		public RouteInformation GetRoute(string start, string end)
		{
			...
			return _cacheProvider.Retrieve<RouteInformation>(string.Format(RouteCacheKey, start.GetHashCode(), end.GetHashCode()), getRoute, CacheLength);
		}
