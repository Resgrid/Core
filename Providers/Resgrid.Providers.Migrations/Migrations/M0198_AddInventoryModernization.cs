...
{
	[Migration(198)]
	public class M0198_AddInventoryModernization : Migration
	{
		...
		private void SpecialIndexes()
		{
			...
			Execute.Sql("CREATE UNIQUE INDEX UX_InventoryLocations_Default ON InventoryLocations(DepartmentId) WHERE IsDefault = 1 AND IsDeleted = 0; CREATE UNIQUE INDEX UX_InventoryKitItems_Item ON InventoryKitItems(DepartmentId,KitId,ItemId) WHERE IsDeleted = 0; CREATE UNIQUE INDEX UX_InventoryIssuances_OutstandingAsset ON InventoryIssuances(DepartmentId,AssetId) WHERE AssetId IS NOT NULL AND IsDeleted = 0 AND Status IN (0,2);");
			foreach (var column in new[] { "GroupId", "UnitId", "UserId", "ContainerAssetId" })
				Execute.Sql("CREATE UNIQUE INDEX UX_InventoryLocations_" + column + " ON InventoryLocations(DepartmentId," + column + ") WHERE " + column + " IS NOT NULL AND IsDeleted = 0;");
		}
		public override void Down()
		{
			// An operator must use the authorized retention path; rollback must never erase ledger/equipment evidence.
			Execute.Sql("IF " + string.Join(" OR ", Tables.Select(t => "EXISTS (SELECT 1 FROM " + N(t) + ")")) + " THROW 51000, 'Inventory data must be exported and removed through the authorized retention process before rollback.', 1;");
