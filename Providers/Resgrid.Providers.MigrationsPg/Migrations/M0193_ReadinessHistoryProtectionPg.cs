...
{
	[Migration(193)]
	public class M0193_ReadinessHistoryProtectionPg : Migration
	{
			...
			Execute.WithConnection((connection, transaction) =>
			{
				...
				while (true)
				{
					...
					using (var select = connection.CreateCommand())
					{
						...
						select.CommandText = $"SELECT {Q("DomainEventOutboxId")}, {Q("PayloadJson")}, {Q("AggregateId")} FROM {Q("DomainEventOutbox")} WHERE {Q("ProducerSubsystem")} = 'Checklists' AND {Q("ReadinessRoutingJson")} IS NULL AND {Q("DomainEventOutboxId")} > @cursor ORDER BY {Q("DomainEventOutboxId")} LIMIT 200";
