...
{
	[Migration(196)]
	public class M0196_CompleteChecklistSchedulingPg : Migration
	{
		...
		public override void Down()
		{
			Execute.WithConnection((connection, transaction) =>
			{
				...
				command.CommandText = $"SELECT (SELECT COUNT(*) FROM {Q("ChecklistSchedules")} WHERE {Q("AssignmentType")}<>0) + (SELECT COUNT(*) FROM {Q("ChecklistReminders")} WHERE {Q("PeriodKey")}<>'') + (SELECT COUNT(*) FROM {Q("DepartmentChecklistSettings")} WHERE {Q("DigestActiveFromUtc")} IS NOT NULL OR {Q("NotifyAtShiftStart")} = false OR {Q("LastDigestSweepUtc")} IS NOT NULL OR {Q("FixedDigestMinute")} IS NOT NULL)";
