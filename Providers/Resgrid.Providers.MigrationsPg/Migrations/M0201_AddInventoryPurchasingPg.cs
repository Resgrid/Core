...
{
	[Migration(201)]
	public class M0201_AddInventoryPurchasingPg : Migration
	{
		...
		public override void Up()
		{
			...
			foreach (var name in Tables)
			{
				...
				switch (name)
				{
					case "InventoryVendors":
						table.WithColumn(N("ContactId")).AsCustom("citext").NotNullable();
						break;
			...
			Execute.Sql("CREATE UNIQUE INDEX ux_inventoryvendors_activecontact ON inventoryvendors(departmentid,contactid) WHERE isdeleted = false;");
			Execute.Sql("CREATE UNIQUE INDEX ux_inventorypurchaseorderitems_activeline ON inventorypurchaseorderitems(departmentid,purchaseorderid,linenumber) WHERE isdeleted = false;");
		...
		public override void Down()
		{
			...
			Execute.Sql("DO $guard$ BEGIN IF " + string.Join(" OR ", Tables.Select(t => "EXISTS (SELECT 1 FROM " + N(t) + ")"))
				+ " OR EXISTS (SELECT 1 FROM inventorytransactions WHERE purchaseorderitemid IS NOT NULL) THEN RAISE EXCEPTION 'Inventory purchasing data must be exported and removed through the authorized retention process before rollback.'; END IF; END $guard$;");
