...
{
	[Migration(202)]
	public class M0202_AddInventoryCountsAndAlertsPg : Migration
	{
		...
		public override void Up()
		{
			...
			Check("InventoryCounts", "Fingerprint", "CHAR_LENGTH(SnapshotFingerprint) = 64");
			...
			Check("InventoryAlerts", "DedupKey", "CHAR_LENGTH(DedupKey) = 64");
		...
		{
			Execute.Sql("CREATE UNIQUE INDEX ux_inventorycountitems_position ON inventorycountitems(departmentid,countid,itemid,locationid,COALESCE(lotid, '00000000-0000-0000-0000-000000000000'),COALESCE(assetid, '00000000-0000-0000-0000-000000000000'));");
		...
		public override void Down()
		{
			...
			Execute.Sql("DO $guard$ BEGIN IF " + string.Join(" OR ", Tables.Select(t => "EXISTS (SELECT 1 FROM " + N(t) + ")"))
				+ " OR EXISTS (SELECT 1 FROM inventorytransactions WHERE countitemid IS NOT NULL) THEN RAISE EXCEPTION 'Inventory count and alert data must be exported and removed through the authorized retention process before rollback.'; END IF; END $guard$;");
