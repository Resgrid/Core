...
namespace Resgrid.Providers.NumberProvider
{
	public class OutboundVoiceProvider : IOutboundVoiceProvider
	{
		...
		public async Task<bool> SendCommunicationTestCallAsync(string phoneNumber, string responseToken)
		{
			if (string.IsNullOrWhiteSpace(phoneNumber) || string.IsNullOrWhiteSpace(responseToken))
				return false;
			...
			try
			{
				...
				var options = new CreateCallOptions(new PhoneNumber(phoneNumber), new PhoneNumber(fromNumber));
				options.Url = new Uri(string.Format(Config.NumberProviderConfig.TwilioCommunicationTestApiUrl, responseToken));
				options.Method = "GET";
