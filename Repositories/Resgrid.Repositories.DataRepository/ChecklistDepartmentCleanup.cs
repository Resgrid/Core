...
namespace Resgrid.Repositories.DataRepository
{
	...
	public static class ChecklistDepartmentCleanup
	{
		public static async Task DeleteWithinTransactionAsync(DbConnection connection, DbTransaction transaction, int departmentId, DatabaseTypes type, CancellationToken ct = default)
		{
			...
			// Phase C children without a DepartmentId column are scoped through their parent.
			if (await Exists("RateScheduleEntryBands")) await connection.ExecuteAsync(new CommandDefinition($"DELETE FROM {Q("RateScheduleEntryBands")} WHERE {Q("RateScheduleEntryId")} IN (SELECT {Q("RateScheduleEntryId")} FROM {Q("RateScheduleEntries")} WHERE {Q("DepartmentId")}=@DepartmentId)", parameters, transaction, cancellationToken: ct));
			if (await Exists("ServiceContractDocumentRequirements")) await connection.ExecuteAsync(new CommandDefinition($"DELETE FROM {Q("ServiceContractDocumentRequirements")} WHERE {Q("ServiceContractId")} IN (SELECT {Q("ServiceContractId")} FROM {Q("ServiceContracts")} WHERE {Q("DepartmentId")}=@DepartmentId)", parameters, transaction, cancellationToken: ct));
