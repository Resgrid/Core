...
namespace Resgrid.Repositories.DataRepository.Queries.Workflows
{
	public class SelectPendingRunsByDepartmentIdQuery : ISelectQuery
	{
		...
		public string GetQuery() =>
			$"SELECT * FROM {_sqlConfiguration.SchemaName}.[WorkflowRuns] WHERE [DepartmentId] = {_sqlConfiguration.ParameterNotation}DepartmentId AND [Status] IN (0, 1) ORDER BY [StartedOn] DESC";
