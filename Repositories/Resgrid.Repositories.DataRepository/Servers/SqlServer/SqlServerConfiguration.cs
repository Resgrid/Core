	...
	public class SqlServerConfiguration : SqlConfiguration
	{
		public SqlServerConfiguration()
		{
			...
			SelectMessagesByDIdQuery = @"
					...
					FROM %SCHEMA%.%MESSAGESTABLE% m
					LEFT JOIN %SCHEMA%.%MESSAGERECIPIENTSTABLE% mr ON mr.[MessageId] =  m.[MessageId]
					WHERE m.[DepartmentId] = %DID% AND m.[IsDeleted] = 0";
			...
			SelectReportCallsCountQuery =
				"SELECT COUNT(*) FROM %SCHEMA%.%TABLENAME% WHERE (%ALLDEPTS% = 1 OR [DepartmentId] = %DID%) AND [IsDeleted] = 0 AND [LoggedOn] >= %STARTDATE% AND [LoggedOn] <= %ENDDATE%";
			...
			SelectReportActiveCallsCountQuery =
				"SELECT COUNT(*) FROM %SCHEMA%.%TABLENAME% WHERE (%ALLDEPTS% = 1 OR [DepartmentId] = %DID%) AND [IsDeleted] = 0 AND [State] = 0";
			...
			SelectReportCallsByMonthQuery =
				"SELECT DATEFROMPARTS(YEAR([LoggedOn]), MONTH([LoggedOn]), 1) AS [Bucket], COUNT(*) AS [Total] FROM %SCHEMA%.%TABLENAME% WHERE (%ALLDEPTS% = 1 OR [DepartmentId] = %DID%) AND [IsDeleted] = 0 AND [LoggedOn] >= %STARTDATE% AND [LoggedOn] <= %ENDDATE% GROUP BY YEAR([LoggedOn]), MONTH([LoggedOn])";
			SelectReportCallsByTypeQuery =
				"SELECT [Type] AS [GroupKey], COUNT(*) AS [Total] FROM %SCHEMA%.%TABLENAME% WHERE (%ALLDEPTS% = 1 OR [DepartmentId] = %DID%) AND [IsDeleted] = 0 AND [LoggedOn] >= %STARTDATE% AND [LoggedOn] <= %ENDDATE% GROUP BY [Type]";
			SelectReportCallsByPriorityQuery =
				"SELECT [Priority] AS [GroupKey], COUNT(*) AS [Total] FROM %SCHEMA%.%TABLENAME% WHERE (%ALLDEPTS% = 1 OR [DepartmentId] = %DID%) AND [IsDeleted] = 0 AND [LoggedOn] >= %STARTDATE% AND [LoggedOn] <= %ENDDATE% GROUP BY [Priority]";
			SelectReportCallsByStateQuery =
				"SELECT [State] AS [GroupKey], COUNT(*) AS [Total] FROM %SCHEMA%.%TABLENAME% WHERE (%ALLDEPTS% = 1 OR [DepartmentId] = %DID%) AND [IsDeleted] = 0 AND [LoggedOn] >= %STARTDATE% AND [LoggedOn] <= %ENDDATE% GROUP BY [State]";
			...
			SelectReportMessagesCountQuery =
				"SELECT COUNT(DISTINCT m.[MessageId]) FROM %SCHEMA%.%TABLENAME% m LEFT JOIN %SCHEMA%.%MEMBERSTABLE% dm ON dm.[UserId] = m.[SendingUserId] WHERE (%ALLDEPTS% = 1 OR dm.[DepartmentId] = %DID%) AND m.[IsDeleted] = 0 AND m.[SentOn] >= %STARTDATE% AND m.[SentOn] <= %ENDDATE%";
			SelectReportMessagesByDayQuery =
				"SELECT CAST(m.[SentOn] AS date) AS [Bucket], COUNT(DISTINCT m.[MessageId]) AS [Total] FROM %SCHEMA%.%TABLENAME% m LEFT JOIN %SCHEMA%.%MEMBERSTABLE% dm ON dm.[UserId] = m.[SendingUserId] WHERE (%ALLDEPTS% = 1 OR dm.[DepartmentId] = %DID%) AND m.[IsDeleted] = 0 AND m.[SentOn] >= %STARTDATE% AND m.[SentOn] <= %ENDDATE% GROUP BY CAST(m.[SentOn] AS date)";
			SelectReportMessagesByMonthQuery =
				"SELECT DATEFROMPARTS(YEAR(m.[SentOn]), MONTH(m.[SentOn]), 1) AS [Bucket], COUNT(DISTINCT m.[MessageId]) AS [Total] FROM %SCHEMA%.%TABLENAME% m LEFT JOIN %SCHEMA%.%MEMBERSTABLE% dm ON dm.[UserId] = m.[SendingUserId] WHERE (%ALLDEPTS% = 1 OR dm.[DepartmentId] = %DID%) AND m.[IsDeleted] = 0 AND m.[SentOn] >= %STARTDATE% AND m.[SentOn] <= %ENDDATE% GROUP BY YEAR(m.[SentOn]), MONTH(m.[SentOn])";
			...
			SelectCallUnitDispatchesForCallsInRangeQuery = @"
					...
					WHERE c.[DepartmentId] = %DID% AND c.[IsDeleted] = 0 AND c.[LoggedOn] >= %STARTDATE% AND c.[LoggedOn] <= %ENDDATE%";
			...
			SelectCallDispatchRolesForCallsInRangeQuery = @"
					...
					WHERE c.[DepartmentId] = %DID% AND c.[IsDeleted] = 0 AND c.[LoggedOn] >= %STARTDATE% AND c.[LoggedOn] <= %ENDDATE%";
			SelectUnitDispatchWindowsQuery = @"
					SELECT cdu.[CallId], cdu.[UnitId], cdu.[DispatchedOn], c.[LoggedOn], c.[ClosedOn]
					FROM %SCHEMA%.%CALLDISPATCHUNITSTABLE% cdu
					INNER JOIN %SCHEMA%.%CALLSTABLE% c ON c.[CallId] = cdu.[CallId]
			...
			SelectPersonnelDispatchWindowsQuery = @"
					SELECT cd.[CallId], cd.[UserId], cd.[DispatchedOn], c.[LoggedOn], c.[ClosedOn], CAST(0 AS bit) AS [Paged]
					FROM %SCHEMA%.%CALLDISPATCHESTABLE% cd
					INNER JOIN %SCHEMA%.%CALLSTABLE% c ON c.[CallId] = cd.[CallId]
					WHERE c.[DepartmentId] = %DID% AND c.[IsDeleted] = 0 AND c.[LoggedOn] >= %LOGGEDFROM% AND c.[LoggedOn] <= %ENDDATE%
						AND cd.[DispatchedOn] >= %STARTDATE% AND cd.[DispatchedOn] <= %ENDDATE%
					UNION ALL
					SELECT cg.[CallId], gm.[UserId], cg.[DispatchedOn], c.[LoggedOn], c.[ClosedOn], CAST(1 AS bit)
					FROM %SCHEMA%.%CALLDISPATCHGROUPSTABLE% cg
					INNER JOIN %SCHEMA%.%CALLSTABLE% c ON c.[CallId] = cg.[CallId]
					INNER JOIN %SCHEMA%.%DEPARTMENTGROUPMEMBERSTABLE% gm ON gm.[DepartmentGroupId] = cg.[DepartmentGroupId]
					WHERE c.[DepartmentId] = %DID% AND c.[IsDeleted] = 0 AND c.[LoggedOn] >= %LOGGEDFROM% AND c.[LoggedOn] <= %ENDDATE%
						AND cg.[DispatchedOn] >= %STARTDATE% AND cg.[DispatchedOn] <= %ENDDATE%
					UNION ALL
					SELECT cr.[CallId], ru.[UserId], cr.[DispatchedOn], c.[LoggedOn], c.[ClosedOn], CAST(1 AS bit)
					FROM %SCHEMA%.%CALLDISPATCHROLESTABLE% cr
					INNER JOIN %SCHEMA%.%CALLSTABLE% c ON c.[CallId] = cr.[CallId]
					INNER JOIN %SCHEMA%.%PERSONNELROLEUSERSTABLE% ru ON ru.[PersonnelRoleId] = cr.[RoleId]
					WHERE c.[DepartmentId] = %DID% AND c.[IsDeleted] = 0 AND c.[LoggedOn] >= %LOGGEDFROM% AND c.[LoggedOn] <= %ENDDATE%
						AND cr.[DispatchedOn] >= %STARTDATE% AND cr.[DispatchedOn] <= %ENDDATE%";
			...
			SelectIndoorMapFloorsByMapIdQuery = @"
					SELECT *
					FROM %SCHEMA%.%TABLENAME%
					WHERE [IndoorMapId] = %MAPID% AND [IsDeleted] = 0
					ORDER BY [FloorOrder]";
			SelectIndoorMapZonesByFloorIdQuery = @"
					SELECT *
					FROM %SCHEMA%.%TABLENAME%
					WHERE [IndoorMapFloorId] = %FLOORID% AND [IsDeleted] = 0";
			SearchIndoorMapZonesQuery = @"
					...
					AND z.[IsSearchable] = 1
					AND z.[IsDeleted] = 0
					AND f.[IsDeleted] = 0
					AND m.[IsDeleted] = 0
					AND z.[Name] LIKE '%' + %TERM% + '%'";
			...
			SelectActiveRoutePlansByDepartmentIdQuery = @"
					SELECT *
					FROM %SCHEMA%.%TABLENAME%
					WHERE [DepartmentId] = %DID% AND [RouteStatus] = 1 AND [IsDeleted] = 0";
			SelectRouteStopsByRoutePlanIdQuery = @"
					SELECT *
					FROM %SCHEMA%.%TABLENAME%
					WHERE [RoutePlanId] = %RPID% AND [IsDeleted] = 0
					ORDER BY [StopOrder]";
			...
			SelectActiveSchedulesDueQuery = @"
					SELECT *
					FROM %SCHEMA%.%TABLENAME%
					WHERE [IsActive] = 1 AND [EffectiveFrom] <= %ASOF%
			...
			SelectUnacknowledgedRouteDeviationsByDepartmentQuery = @"
					...
					WHERE i.[DepartmentId] = %DID% AND d.[IsAcknowledged] = 0
			...
			SelectMatchingCheckInTimerOverridesQuery = @"
				SELECT *
				FROM %SCHEMA%.%TABLENAME%
				WHERE [DepartmentId] = %DID% AND [IsEnabled] = 1
			...
			SelectActiveUdfDefinitionByDeptAndTypeQuery = @"
					SELECT *
					FROM %SCHEMA%.%TABLENAME%
					WHERE [DepartmentId] = %DID% AND [EntityType] = %ENTITYTYPE% AND [IsActive] = 1";
