...
namespace Resgrid.Tests.Services
{
	...
	public sealed class InventoryWorkflowTests
	{
		...
		public async Task Invalid_scalar_types_and_policy_reintroduced_authored_fields_cannot_escape_allowlist()
		{
			...
			payload["ReferenceId"] = "person-" + Canary; payload["OccurredOn"] = Canary;
			...
			foreach (var name in new[] { "Quantity", "ItemId", "OldStatus", "ReferenceId", "OccurredOn", "UserId" }) safe.Property(name).Should().BeNull(name + " is not a valid routing value");
		...
		[Test]
		public async Task Reprojection_after_enrollment_or_deactivation_never_restores_authored_content()
		{
			var safe = await InventoryWorkflowPayload.ProjectAsync(42, Wrapped(22), _projection, true);
			_policy.Setup(s => s.IsProtectionEnforcedAsync(42)).ReturnsAsync(true);
			...
			payload.Value<decimal>("Quantity").Should().Be(2.125001m); payload.Value<string>("ItemName").Should().Be("REDACTED");
		...
		public async Task Inventory_outbox_protects_history_and_replays_only_structural_payload_without_broker_decryption(int trigger)
		{
			...
			entry.ReadinessRoutingJson.Should().NotContain(Canary).And.Contain(TransactionId);
			_history.Decrypt(42, "domaineventoutbox.payloadjson", entry.DomainEventOutboxId.ToString(), entry.PayloadJson).Should().NotContain(Canary).And.Contain("2.125001");
			...
			delivered.ProducerSubsystem.Should().Be("Inventory"); delivered.AggregateId.Should().Be(AssetId); delivered.CorrelationId.Should().Be(CorrelationId);
			delivered.PayloadJson.Should().NotContain(Canary).And.NotContain("rgdp:"); JObject.Parse(delivered.PayloadJson).Value<decimal>("Quantity").Should().Be(2.125001m);
			...
			Template.Parse("{{ event.id }}|{{ inventory.transaction_id }}|{{ inventory.item_name }}").Render(context).Should().Be(entry.EventId + "|" + TransactionId + "|REDACTED");
			_history.Broker.Invocations.Should().OnlyContain(i => i.Method.Name == "EncryptAsync");
		}

		[Test]
		public async Task Unknown_protection_policy_prevents_inventory_outbox_initialization()
		{
			_policy.Setup(s => s.IsProtectionEnforcedAsync(42)).ThrowsAsync(new InvalidOperationException("policy unavailable"));
			await ((Func<Task>)(() => _outbox.EnqueueAsync(42, "Inventory", Event(22)))).Should().ThrowAsync<InvalidOperationException>();
		...
		public async Task Queue_rejection_retries_existing_inventory_workflow_run_with_same_event_identity(int trigger)
		{
			...
			queued.Value<string>("AggregateId").Should().Be(AssetId); attempts[1].EventPayloadJson.Should().NotContain(Canary).And.NotContain("rgdp:");
		...
		public async Task Duplicate_consumer_claim_does_not_execute_completed_inventory_run(int trigger)
		{
			...
			claimed.Should().StartWith("rgdp:"); _history.Decrypt(42, "workflowruns.inputpayload", run.WorkflowRunId, claimed).Should().NotContain(Canary).And.Contain(TransactionId);
		...
		private static JObject Wrapped(int trigger) => new() { ["DepartmentId"] = 42, ["EventId"] = "77777777-7777-7777-7777-777777777777",
			["EventName"] = ((WorkflowTriggerEventType)trigger).ToString(), ["TriggerEventType"] = trigger, ["SchemaVersion"] = 1, ["AggregateType"] = "InventoryAsset", ["AggregateId"] = AssetId,
			...
			["Payload"] = Payload(), ["UnreviewedEnvelopeText"] = Canary };
		private static DomainEventEnvelope Event(int trigger) => new() { EventName = ((WorkflowTriggerEventType)trigger).ToString(), SchemaVersion = 1,
			AggregateType = "InventoryAsset", AggregateId = AssetId, Trigger = (WorkflowTriggerEventType)trigger, CorrelationId = CorrelationId, Payload = Payload() };
	}
}
