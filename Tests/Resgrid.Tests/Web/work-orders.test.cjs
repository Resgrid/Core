...
<script type="application/json" id="work-order-page">{"protectedData":true,"grant":"synthetic-grant","expiry":"2030-01-01T00:00:00Z","reopen":"/User/WorkOrders/Reopen","index":"/User/WorkOrders","error":"Erreur"}</script>`);
        await page.evaluate(()=>{
            ...
            window.fetch=async(url,options)=>{window.requests.push(Object.fromEntries(options.body));return window.fail ? {ok:false,json:async()=>({message:'Conflit',code:'Conflict'})}:{ok:true,json:async()=>({id:123})};};
