...
namespace Resgrid.Workers.Framework.Logic
{
	...
	/// tick where its local hour equals CertificationConfig.SweepLocalHour, so "today", the lead days and the grace
	/// deadlines are all department-local dates and every department sees exactly one sweep per local day. Only
	/// departments with typed personnel records, unit records or role requirements are visited; one department's
