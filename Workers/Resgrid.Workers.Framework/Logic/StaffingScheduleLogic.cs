			...
			if (item != null && item.ScheduledTask != null)
			{
				try
				{
					...
					else if (item.ScheduledTask.TaskType == (int)TaskTypes.DepartmentStaffingReset)
					{
						...
						{
							await _userStateService.CreateUserState(user.UserId, item.ScheduledTask.DepartmentId, int.Parse(item.ScheduledTask.Data), $"Department Staffing Reset {item.ScheduledTask.ScheduledTaskId}", autoGenerated: true);
						}
					}
				}
				catch (Exception ex)
